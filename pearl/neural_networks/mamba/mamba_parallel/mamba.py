...
from pearl.neural_networks.mamba.mamba_parallel.pscan import pscan
...
class Mamba(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.embedding = nn.Linear(config.input_dim, config.d_model, bias=config.bias)
        self.layers = nn.ModuleList([ResidualBlock(config) for _ in range(config.n_layers)])
        self.norm_f = RMSNorm(config.d_model)
        self.state_proj = nn.Linear(config.d_model, config.state_dim, bias=config.bias)

    def forward(self, x):
        """
        Args:
            input (float tensor): shape (b, l, input_dim)    (See Glossary at top for definitions of b, l, d_in, n...)

        Returns:
            logits: shape (b, l, state_dim)
        ...
        if self.config.state_dim != self.config.d_model:
            logits = self.state_proj(x)
...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        """
        Args:
            input (float tensor): shape (b, l, d_model)    (See Glossary at top for definitions of b, l, d_in, n...)
